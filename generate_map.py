...
<style>
  ...
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  /* ── Intro banner (onboarding) ────────────────────────────── */
  #intro-banner {{
    position: fixed; inset: 0; z-index: 3000;
    background: rgba(5,5,16,0.97); backdrop-filter: blur(8px);
  ...
  .intro-box {{
    ...
    width: min(540px, 96vw); border-radius: var(--radius); overflow: hidden;
  }}
  .intro-head {{
    background: var(--panel2); padding: 24px 28px 16px;
    border-bottom: 1px solid rgba(255,255,255,0.06); text-align: center;
  }}
  .intro-logo {{
    font-family: var(--display); font-size: 1.2rem; font-weight: 900;
    color: var(--neon); letter-spacing: 5px; margin-bottom: 6px;
  }}
  .intro-tagline {{
    font-family: var(--mono); font-size: 0.58rem; letter-spacing: 2px;
    color: rgba(255,255,255,0.3);
  }}
  .intro-body {{ padding: 20px 28px 24px; }}
  .intro-desc {{
  ...
  .intro-feat strong {{ color: var(--neon); font-weight: 600; display: block; margin-bottom: 2px; font-size: 0.72rem; }}
  .intro-btn {{
    width: 100%; background: var(--neon); color: #050510;
    border: none; font-family: var(--display); font-weight: 700;
    font-size: 0.72rem; letter-spacing: 3px; padding: 13px 24px;
    border-radius: var(--radius); cursor: pointer;
    box-shadow: 0 0 24px var(--neon-glow); transition: opacity 0.2s;
  }}
  .intro-btn:hover {{ opacity: 0.85; }}
  .intro-remember {{
    display: flex; align-items: center; gap: 8px; margin-bottom: 12px;
    font-size: 0.62rem; color: rgba(255,255,255,0.35); font-family: var(--mono);
    cursor: pointer; justify-content: center;
  }}
  .intro-remember input {{ accent-color: var(--neon); }}

  /* ── Map loading overlay ───────────────────────────────────── */
  #map-loading {{
    position: absolute; inset: 0; z-index: 50;
    background: rgba(5,5,16,0.82); backdrop-filter: blur(2px);
  ...
  .loading-label {{
    font-family: var(--mono); font-size: 0.58rem; letter-spacing: 3px;
    color: var(--neon); animation: blink 1.2s infinite;
  }}

  /* ── Share button ──────────────────────────────────────────── */
  .share-btn {{
    background: transparent; border: 1px solid var(--border);
    color: var(--dim2); font-family: var(--mono); font-size: 0.58rem;
    letter-spacing: 1px; padding: 3px 10px; border-radius: 2px;
    transition: all 0.2s; cursor: pointer;
  }}
  ...
  /* ── Stat context badge ────────────────────────────────────── */
  .stat-ctx {{
    font-size: 0.5rem; font-family: var(--mono); padding: 1px 5px;
    border-radius: 2px; display: inline-block; margin-top: 3px;
  }}
  .ctx-high {{ background:rgba(255,68,68,0.1); color:#ff6666; border:1px solid rgba(255,68,68,0.22); }}
  .ctx-low  {{ background:rgba(0,255,65,0.07); color:#00dd38; border:1px solid rgba(0,255,65,0.18); }}
  .ctx-avg  {{ background:rgba(255,255,255,0.04); color:rgba(255,255,255,0.35); border:1px solid rgba(255,255,255,0.09); }}

  /* ── Korelasi narasi ───────────────────────────────────────── */
  .korelasi-narasi {{
    background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07);
    border-radius: var(--radius); padding: 10px 12px; margin: 8px 0 12px;
    font-size: 0.67rem; color: rgba(255,255,255,0.55); line-height: 1.65;
  }}
  .korelasi-narasi em {{ color: var(--neon); font-style: normal; font-weight: 600; }}
  ...
  #toast {{
    ...
    color: var(--neon); padding: 8px 20px; border-radius: 2px;
    z-index: 9999; pointer-events: none;
    opacity: 0; transition: opacity 0.3s;
    white-space: nowrap; box-shadow: 0 0 16px var(--neon-glow);
  }}
  ...
  @media (max-width: 768px) {{
    aside {{
      position: fixed; top: 48px; left: 0; bottom: 0; width: 85vw; max-width: 300px;
      transform: translateX(-100%); z-index: 300;
      box-shadow: 4px 0 24px rgba(0,0,0,0.7);
    }}
    aside.open {{ transform: translateX(0); }}
    #sidebar-toggle {{ display: flex; }}
    .logo {{ font-size: 0.78rem; letter-spacing: 3px; }}
    .logo-sub {{ display: none; }}
    .hud-chip {{ display: none; }}
    .hud-chip:first-child {{ display: flex; }}
    #clk {{ display: none; }}
    .metod-btn {{ display: none; }}
    .share-btn {{ display: none; }}
    #art-panel {{ width: 100vw; right: -100vw; }}
    #map-hint {{ font-size: 0.6rem; padding: 6px 14px; bottom: 70px; }}
    .intro-features {{ grid-template-columns: 1fr; }}
  }}
  @media (max-width: 480px) {{
    header {{ padding: 0 12px; height: 44px; }}
    .intro-box {{ margin: 0; }}
    .intro-head {{ padding: 18px 20px 12px; }}
    .intro-body {{ padding: 16px 20px 20px; }}
    .intro-logo {{ font-size: 1rem; }}
    .intro-desc {{ font-size: 0.78rem; }}
  }}
</style>
</head>
<body>
...
<div id="intro-banner">
  <div class="intro-box">
    <div class="intro-head">
      <div class="intro-logo">NEXUS // MAP</div>
      <div class="intro-tagline">INDONESIA CRIME &amp; HEALTH SURVEILLANCE</div>
    </div>
    <div class="intro-body">
      <p class="intro-desc">
        Dashboard interaktif untuk memantau <strong style="color:var(--neon)">kriminalitas, kesehatan, dan bencana alam</strong>
        di 34 provinsi Indonesia — berbasis data resmi BPS, Kemenkes, BNPB, dan BNN 2023.
      </p>
      <div class="intro-features">
        <div class="intro-feat"><strong>&#9670; 5 Layer Data</strong>Kriminalitas, kekerasan seksual, penyakit menular, bencana alam, narkoba</div>
        <div class="intro-feat"><strong>&#9632; Peta Interaktif</strong>Klik provinsi untuk melihat berita terkait dari wilayah tersebut</div>
        <div class="intro-feat"><strong>&#9651; Analisis Korelasi</strong>Hubungan kemiskinan, IPM, dan angka kejadian per provinsi</div>
        <div class="intro-feat"><strong>&#9679; Berita Real-time</strong>Artikel dari 40+ sumber RSS yang diperbarui otomatis</div>
      </div>
      <label class="intro-remember">
        <input type="checkbox" id="intro-skip-cb"/> Jangan tampilkan lagi
      </label>
      <button class="intro-btn" onclick="closeIntroBanner()">MULAI EKSPLORASI &#8594;</button>
    </div>
  </div>
</div>
...
<header>
  ...
  <div class="hud-right">
    ...
    <button class="share-btn" onclick="shareMap()" title="Salin tautan peta ini">&#128279; BAGIKAN</button>
...
<div class="main">
  ...
  <aside id="sidebar">
    ...
    <div class="tab-pane" id="tab-stats">
      ...
      <div class="stats-grid">
        <div class="stat-card wide">
          <div class="stat-lbl">Total Kasus Nasional</div>
          <div class="stat-val" id="s-total">—</div>
        </div>
        <div class="stat-card">
          <div class="stat-lbl">Rata-rata/Prov</div>
          <div class="stat-val" id="s-avg">—</div>
          <div id="s-avg-ctx"></div>
        </div>
        <div class="stat-card" title="Provinsi dengan jumlah kasus tertinggi">
          <div class="stat-lbl">Tertinggi (absolut)</div>
          <div class="stat-val" id="s-max">—</div>
          <div id="s-max-ctx" style="font-size:0.52rem;color:rgba(255,255,255,0.3);margin-top:2px;"></div>
        </div>
        <div class="stat-card" title="Rata-rata per 100.000 penduduk. Makin tinggi = makin berisiko per kapita. Populasi: BPS Proyeksi 2023">
          <div class="stat-lbl">Per 100rb Jiwa &#9432;</div><div class="stat-val" id="s-per100k">—</div>
          <div id="s-per100k-ctx"></div>
        </div>
    ...
    <div class="tab-pane" id="tab-korelasi">
      ...
      <!-- Narasi otomatis -->
      <div id="korelasi-narasi-1" class="korelasi-narasi">Menghitung korelasi...</div>
      ...
      <div id="korelasi-narasi-2" class="korelasi-narasi" style="margin-top:4px"></div>
      <div style="margin-bottom:4px;font-size:0.65rem;color:var(--neon)">
  ...
  <div class="map-wrap">
    ...
    <div id="map-loading">
      ...
      <div class="loading-label">MEMUAT PETA...</div>
    </div>
    <iframe id="map-frame" src="maps/kriminalitas.html" title="Peta interaktif"></iframe>
    <div id="map-hint">&#9650;&nbsp; Klik provinsi untuk lihat berita &nbsp;|&nbsp; &#9776; Buka menu kiri untuk ganti layer</div>
...
function updateStats(s, neon, key) {{
  ...
  // Provinsi tertinggi di stat-max
  const maxCtxEl = document.getElementById('s-max-ctx');
  if (maxCtxEl && s.top && s.top.length) maxCtxEl.textContent = s.top[0][0];

  // Context: berapa kali lipat dari rata-rata nasional
  _setCtx('s-avg-ctx', s.avg||0, (s.total||0)/34);
  _setCtx('s-per100k-ctx', s.per100k_avg||0, s.per100k_avg||0);
...
function closeIntroBanner() {{
  ...
  const cb = document.getElementById('intro-skip-cb');
  if (cb && cb.checked) {{
    try {{ localStorage.setItem('nexus_intro_seen', '1'); }} catch(e) {{}}
  }}
}}
(function initIntroBanner() {{
  try {{
    if (localStorage.getItem('nexus_intro_seen') === '1') {{
      const b = document.getElementById('intro-banner');
...
function showToast(msg, duration) {{
  ...
  setTimeout(() => el.classList.remove('show'), duration || 2800);
}}

// ── Share / copy link ────────────────────────────────────────────
function shareMap() {{
  const base = window.location.href.split('?')[0].split('#')[0];
  const url  = base + '?layer=' + currentKey;
  try {{
    navigator.clipboard.writeText(url).then(() => showToast('&#10003; Tautan disalin: ?' + currentKey));
  }} catch(e) {{
    // fallback
...
// Baca parameter ?layer= dari URL saat load
(function applyUrlLayer() {{
  try {{
    const p = new URLSearchParams(window.location.search);
    const l = p.get('layer');
    if (l && window.LAYERS && window.LAYERS[l]) {{
      window._initialLayer = l;
    }}
  }} catch(e) {{}}
}})();

// ── Stat context badges ──────────────────────────────────────────
function _setCtx(elId, val, avg, unit) {{
  const el = document.getElementById(elId);
  if (!el || !avg) return;
  const ratio = val / avg;
  let cls = 'ctx-avg', txt = '≈ rata-rata';
  if (ratio > 1.3)       {{ cls = 'ctx-high'; txt = '+' + Math.round((ratio-1)*100) + '% di atas rata-rata'; }}
  else if (ratio < 0.7)  {{ cls = 'ctx-low';  txt = Math.round((1-ratio)*100) + '% di bawah rata-rata'; }}
  el.innerHTML = '<span class="stat-ctx ' + cls + '">' + txt + '</span>';
}}
...
function _pearson(xs, ys) {{
  ...
  return (sx*sy > 0) ? cov/(sx*sy) : 0;
}}
function _buildKorelasiNarasi() {{
  if (!KORELASI || !KORELASI.length) return;
  const riskMap = {{}};
  RISK_TOP10.forEach(r => riskMap[r[0]] = r[1]);
  const data = KORELASI.map(d => Object.assign({{}}, d, {{risk: riskMap[d.p] || 0}}));

  // Korelasi 1: kemiskinan vs kriminalitas
  const r1 = _pearson(data.map(d=>d.x), data.map(d=>d.y));
  const topKem = [...data].sort((a,b)=>b.x-a.x)[0];
  const topKrim = [...data].sort((a,b)=>b.y-a.y)[0];
  let dir1, dir1txt;
  if (r1 > 0.4)       {{ dir1 = 'positif'; dir1txt = 'Artinya, provinsi dengan kemiskinan lebih tinggi cenderung memiliki kasus kriminalitas per kapita yang lebih tinggi pula.'; }}
  else if (r1 < -0.4) {{ dir1 = 'negatif'; dir1txt = 'Artinya, hubungan antara kemiskinan dan kriminalitas per kapita tidak linear — faktor lain seperti kepadatan kota atau akses layanan turut berperan.'; }}
  else                {{ dir1 = 'lemah';   dir1txt = 'Korelasi tidak kuat — faktor lain seperti kepadatan kota, akses hukum, dan urbanisasi berpengaruh lebih besar terhadap kriminalitas.'; }}
  const n1El = document.getElementById('korelasi-narasi-1');
  if (n1El) n1El.innerHTML =
    'Korelasi kemiskinan–kriminalitas: <em>r = ' + r1.toFixed(2) + ' (' + dir1 + ')</em>. ' + dir1txt +
    ' Provinsi termiskin: <em>' + topKem.p + ' (' + topKem.x + '%)</em>. ' +
    'Kriminalitas/100rb tertinggi: <em>' + topKrim.p + ' (' + topKrim.y + ')</em>.';

  // Korelasi 2: IPM vs risk score
  const r2 = _pearson(data.map(d=>d.ipm), data.map(d=>d.risk));
  const topRisk = [...data].sort((a,b)=>b.risk-a.risk)[0];
  let dir2txt;
  if (r2 < -0.3)      dir2txt = 'Provinsi dengan IPM lebih tinggi cenderung memiliki risk score lebih rendah — kapasitas daerah yang lebih baik mampu meredam dampak ancaman.';
  else if (r2 > 0.3)  dir2txt = 'Hubungan tidak linier — IPM tinggi tidak selalu berarti risiko rendah, kemungkinan karena urbanisasi meningkatkan paparan beberapa kategori risiko.';
  else                dir2txt = 'Hubungan IPM dan risk score tidak kuat secara keseluruhan — distribusi risiko dipengaruhi banyak faktor struktural.';
  const n2El = document.getElementById('korelasi-narasi-2');
  if (n2El) n2El.innerHTML =
    'Korelasi IPM–risiko: <em>r = ' + r2.toFixed(2) + '</em>. ' + dir2txt +
    ' Risk score tertinggi: <em>' + (topRisk ? topRisk.p + ' (' + topRisk.risk.toFixed(1) + ')' : '—') + '</em>.';
}}
...
document.addEventListener('DOMContentLoaded', () => {{
  // Terapkan layer dari URL param jika ada
  const initLayer = window._initialLayer;
  let startBtn = null;
  if (initLayer) {{
    startBtn = document.querySelector('.layer-btn[data-key="' + initLayer + '"]');
  }}
  if (!startBtn) startBtn = document.querySelector('.layer-btn');
  if (startBtn) switchLayer(startBtn.dataset.key, startBtn);
  updateSystemStatus();
  // iframe pertama kali sudah src default — tunggu load lalu hide loading
  const frame = document.getElementById('map-frame');
  const loading = document.getElementById('map-loading');
  if (frame && loading) {{
    frame.addEventListener('load', () => loading.classList.add('hidden'), {{once: true}});
    // Fallback hide jika sudah loaded (cache)
    setTimeout(() => loading && loading.classList.add('hidden'), 4000);
  }}
  // Auto-hide onboarding hint setelah 10 detik
  setTimeout(() => {{
    const hint = document.getElementById('map-hint');
